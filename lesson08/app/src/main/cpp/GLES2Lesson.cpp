...
//start off as identity - late we will init it with proper values.
	...
	void GLES2Lesson::resetTransformMatrices() {
		...
		glm::mat4 rotatedAroundYAxis = glm::rotate(rotatedAroundXAxis, cubeRotationAngleXZ * rads,
												   yAxis);
	...
	void GLES2Lesson::drawGeometry(const int vertexVbo, const int indexVbo, int vertexCount,
		...
		glUniform4fv(ambientLightColorShaderLocation, 1,
					 mFullLight ? &ambientLightFullColor[0] : &ambientLightOffColor[0]);

		glUniformMatrix4fv(modelMatrixAttributePosition, 1, false, &transform[0][0]);

		glVertexAttribPointer(vertexAttributePosition, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 9, nullptr);
	...
	void GLES2Lesson::disableAlfaBlending() {
	...
	void GLES2Lesson::enableAlphaBlending() {
